...
create index idx_product_img on tbl_product_img(pno);

alter table tbl_product_img add constraint fk_product_img 
foreign key (pno) references tbl_product(pno);

select * from tbl_product;

insert into tbl_product_img (pno,img) values (1,'1.jpg');

insert into tbl_product_img (pno,img) values (1,'1-1.jpg');

insert into tbl_product_img (pno,img) values (1,'1-2.jpg');

insert into tbl_product_img (pno,img) values (2,'2.jpg');

insert into tbl_product_img (pno,img) values (3,'3.jpg');

select * 
from
  tbl_product tp left outer join tbl_product_img tpi on tpi.pno = tp.pno
where tp.pno > 0
