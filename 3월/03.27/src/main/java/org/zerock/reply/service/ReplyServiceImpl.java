...
public class ReplyServiceImpl implements ReplyService {
	...
	public PageResponseDTO<ReplyDTO> addReply(ReplyDTO replyDTO) {
		...
		int lastPage = (int)(Math.ceil(replyCount/10.0));
		
		int skip = (lastPage -1) * 10;
	...
	@Override
	public PageResponseDTO<ReplyDTO> list(Integer bno, PageRequestDTO requestDTO, boolean last) {
		
		int total = replyMapper.countOfBno(bno);
		...
		if(last) {
		  	int lastPage = (int)(Math.ceil(total/10.0));  //6,7
		  	requestDTO.setPage(lastPage);
		}
		
		java.util.List<ReplyDTO> replyList =  replyMapper.selectList(bno, requestDTO);
	...
	@Override
	public void modify(ReplyDTO replyDTO, boolean del) {
		
		if(del) {
			replyDTO.setReplyText("삭제되었습니다");
		}
		
		replyMapper.updateOne(replyDTO);
